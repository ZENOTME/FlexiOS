use crate::{
    addr_type::{phys_to_kernel, Addr, PhysAddr},
...
impl FrameObj {
    pub fn frame_size(&self) -> FrameSize {
        match self {
            FrameObj::Data(data) => data.frame_size(),
            FrameObj::Guard(guard) => guard.frame_size(),
            FrameObj::Lazy(lazy) => lazy.frame_size(),
        }
    }
}
...
impl DataFrame {
    ...
    pub fn as_slice<T: FromBytes>(&self, offset: u64, len: u64) -> Result<&[T], &'static str> {
        ...
        let slc: &[T] = unsafe {
            slice::from_raw_parts(
                phys_to_kernel(self.start + offset).addr() as *const T,
                len as usize,
            )
        };
        Ok(slc)
    ...
    pub fn as_slice_mut<T: FromBytes>(
        ...
        let slc: &mut [T] = unsafe {
            slice::from_raw_parts_mut(
                phys_to_kernel(self.start + offset).addr() as *mut T,
                len as usize,
            )
        };
        Ok(slc)
    ...
    pub fn as_type<T: FromBytes>(&self, offset: u64) -> Result<&T, &'static str> {
        ...
        let t: &T = unsafe { &*((phys_to_kernel(self.start).addr() + offset) as *const T) };
        Ok(t)
    }
    ...
    pub fn as_type_mut<T: FromBytes>(&self, offset: u64) -> Result<&mut T, &'static str> {
        ...
        let t: &mut T = unsafe { &mut *((phys_to_kernel(self.start).addr() + offset) as *mut T) };
        Ok(t)
    }
}
